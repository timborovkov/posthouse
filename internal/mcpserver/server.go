...
func (s *Server) registerTools() {
	...
	mcp.AddTool(s.mcp, &mcp.Tool{Name: "messages_send_prepare", Title: "Prepare message send", Description: "Prepare a plain-text or HTML email with up to 25 MiB total attachment data through exactly one mail connection. Text-only is text/plain. HTML-only is multipart/alternative with a derived text/plain fallback. Both bodies are multipart/alternative as supplied. Returns a ten-minute opaque token and exact side-effect preview; no message is sent until operation_execute is called.", Annotations: readOnly},
	...
	mcp.AddTool(s.mcp, &mcp.Tool{Name: "messages_reply_prepare", Title: "Prepare message reply", Description: "Fetch one provider message by opaque id and prepare a threaded plain-text or HTML reply through the same exact connection, honoring Reply-To. No message is sent until operation_execute.", Annotations: readOnly},
	...
	mcp.AddTool(s.mcp, &mcp.Tool{Name: "messages_forward_prepare", Title: "Prepare message forward", Description: "Fetch one provider message by opaque id and prepare a plain-text or HTML forward through the same exact connection. No message is sent until operation_execute.", Annotations: readOnly},
