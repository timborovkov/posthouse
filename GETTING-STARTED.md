...
Set `POSTHOUSE_TRUST_PROXY=1` when a reverse proxy supplies `X-Forwarded-For`, so brute-force lockout keys off the real client rather than the proxy.
...
3. Set `POSTHOUSE_CACHE_KEY`, `POSTHOUSE_ACCESS_KEY`, and your provider secret variables. Railway injects `PORT`; Posthouse listens on it automatically.
