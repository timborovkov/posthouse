...
The encrypted key-check marker makes a wrong cache key a readiness failure. Rekeying holds SQLite's write lock while every encrypted writer verifies that marker, preventing stale processes from persisting old-key ciphertext. Keychain-backed rekey stores the new key in a recovery record encrypted by the old key inside the same SQLite transaction, then promotes the credential after commit; startup completes that promotion after an interruption. Headless operators must replace `POSTHOUSE_CACHE_KEY` with the requested new value immediately after a successful rekey.
