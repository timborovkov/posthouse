...
`MESSAGE_ID` is the opaque `id` from list/search (connection + `id`). Do not invent IMAP `--uid` values for Gmail or Microsoft. Pass `--cursor` from `next_cursor` with identical filters. `--offline` = cache only; `--refresh` refuses stale fallback.
